...
>
> Docker/Kubernetes stage note: if `docker` or `kubectl` CLIs are missing in Jenkins runtime, those stages are marked **UNSTABLE** (not failed) with a clear message so your earlier CI stages still complete.
