...
def scrape_homepage(driver: webdriver.Chrome, base_url: str) -> ScrapeSummary:
    driver.get(base_url)
    ...
    return ScrapeSummary(title=page_title, heading=heading, product_count=product_count)
...
def main() -> int:
    base_url = os.getenv("E2E_BASE_URL", "http://localhost:30080")
    print(f"[selenium-python] starting smoke scrape against: {base_url}")

    driver: webdriver.Chrome | None = None
    try:
        driver = build_driver()
        summary = scrape_homepage(driver, base_url)
        verify_login_page(driver, base_url)
